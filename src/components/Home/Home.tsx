...

      {
       trendingAnime && trendingAnime.map((item:any)=>{
          ...
          <SwiperSlide key={item.id}>
            <div onClick={handleAnimeClick} style={{"cursor": "pointer"}} className='rounded-xl  lg:h-[24rem] h-[14rem] w-full pb-8 flex justify-center relative'>
    ...
    >
      {
       recentAnime && recentAnime.map((item:any)=>{
          return (
          <SwiperSlide key={item.id}>
            <div  onClick={handleAnimeClick}  className="w-full lg:mb-9 relative rounded-2xl " style={{background: `url(${item.image})`, backgroundSize: "cover", backgroundRepeat: "no-repeat",height: "360px"}}>
