...
import { addAnime, animeType } from "../../features/currentAnimeSlice";
...
function Home() {
  ...
  useEffect(() => {
    async function setAnimeDataHome() {
      try {
        ...
        const recentResult = await axios.get(
  ...
  function handleAnimeClick(item: animeType) {
  ...
  return (
    <div className="bg-slate-900 p-3" style={{ minHeight: "100vh" }}>
      ...
      >
        <img
          src={logo}
          className="w-12 lg:w-12  rounded-lg absolute top-5 left-5 z-50"
          alt=""
        />
              ...
              return (
                <SwiperSlide key={item.id}>
                  <div
                    ...
                    className="rounded-xl  lg:h-[24rem] h-[14rem] w-full pb-8 flex justify-center relative"
                  >
                    <div className="relative h-full z-30 w-full flex justify-start items-start">
                      <img
                        ...
                        alt=""
                      />
                      <p className="lg:top-20 main-title text-white font-normal z-20  lg:text-4xl lg:left-10 absolute">
                        {item.title.english}
                      </p>
                      <p className="top-40 lg:w-3/4 crasouelDesc text-white font-noraml z-20 text- left-10 absolute">
                        {cleanHTML(item.description, 700)}
                      </p>
      ...
      >
        {recentAnime &&
          recentAnime.map((item: any) => {
            return (
              <SwiperSlide key={item.id}>
                <div
                  onClick={() => handleAnimeClick}
                  className="w-full lg:mb-9 relative rounded-2xl "
                  style={{
                ...
                >
                  <p className="absolute bottom-3 main-title text-bold px-3 z-20 text-white">
                    {item.title.english && cleanHTML(item.title.english, 26)}
                  </p>
                  <div className="backDropGradient z-10 rounded-xl absolute w-full h-full">
                    .
                  </div>
                </div>
              </SwiperSlide>
